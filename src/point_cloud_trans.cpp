...
using namespace std;
PointcloudTrans::PointcloudTrans(const ros::NodeHandle& n, std::string sensor_topic):nh_(n),tfListener_(tfBuffer_)
{
  cout << "start ... " << endl;
  cout << "sensor_topic: " << sensor_topic << endl;
  cloud1_sub_ = nh_.subscribe(sensor_topic, 10, &PointcloudTrans::HandleCloudFront, this);
  cloud1_pub_ = nh_.advertise<sensor_msgs::PointCloud2>(sensor_topic + "1", 10);
}
...
//   FLAGS_colorlogtostderr = true;
  ...
  cout << "fuck  " << endl;
  string sensor_topic = string(argv[1]);
  ros::start();
  ros::NodeHandle n("~");
  PointcloudTrans a(n, sensor_topic);
  ros::spin();
  return 1;
}